...
SVM_kfold <- lapply(1:5, function(rep) {
  ...
  lapply(1:5, function(x) {
    ...
    tibble(predicted = predict(SVM,ecoCommunitiesOrd[-train,]),
...
SVM_confmat <- SVM_kfold %>% 
  ...
  labs(title = "Support vector machine - Laplacian kernel",
...
ggsave("SVM_confusion_matrix.png", SVM_confmat,
       type = "cairo-png", dpi = 400, width = 6, height = 6, scale = 2)
...
SVM_kfold %>% 
  ...
  as.character %>% 
  {
    write_lines(.,file("SVM_accuracy.txt"))
  }
